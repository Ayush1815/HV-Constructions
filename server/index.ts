...
<body style="margin:0;padding:0;background:#f4f6f8;font-family:system-ui,sans-serif">
      ...
      <table width="100%" style="max-width:560px;background:#fff;border-radius:16px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.08)">
        <tr>
          <td style="background:#805113;padding:24px 32px">
            <p style="margin:0;font-size:11px;font-weight:700;letter-spacing:0.12em;text-transform:uppercase;color:#f2c879">HV Construction</p>
            <h1 style="margin:6px 0 0;font-size:20px;color:#fff;font-weight:800">New Project Inquiry</h1>
          </td>
        </tr>
        <tr>
        ...
        <tr>
          <td style="padding:16px 32px 28px;font-size:12px;color:#94a3b8;border-top:1px solid #f0f0f0">
            This notification was sent automatically by HV Construction when a prospect submitted the project inquiry form.
          </td>
        </tr>
      </table>
    </td></tr>
  ...
  try {
    ...
    const internalEmail = resend.emails.send({
      from: "HV Construction Leads <onboarding@resend.dev>",
      to: [CONTACT_EMAIL],
      subject: `New project inquiry — ${lead.name} (${lead.business})`,
      html,
    });
...
<body style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;line-height:1.6;color:#334155;margin:0;padding:0;background-color:#f8fafc">
  <table width="100%" cellpadding="0" cellspacing="0" style="padding:40px 20px">
      ...
      <table width="600" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:16px;overflow:hidden;border:1px solid #e2e8f0;box-shadow:0 10px 25px -5px rgba(0,0,0,0.05)">
        <tr>
          <td style="background-color:#0f172a;padding:40px 32px;text-align:center">
            <h1 style="margin:0;font-size:24px;color:#ffffff;font-weight:800;letter-spacing:-0.02em">HV <span style="color:#eab308">Construction</span></h1>
          </td>
        </tr>
        <tr>
          <td style="padding:40px 32px">
            <p style="font-size:16px;color:#0f172a;font-weight:600">Hi ${lead.name},</p>
            <p style="font-size:15px;color:#475569">We've successfully received your project inquiry for <strong>${lead.business}</strong>.</p>
            <p style="font-size:15px;color:#475569">Our team is currently reviewing your project requirements. We will assess the scope and reach out to you shortly to discuss the next steps.</p>
            ${process.env.VITE_CALENDLY_URL ? `
            <div style="margin:32px 0;text-align:center">
              <p style="font-size:14px;color:#64748b;margin-bottom:12px">Want to schedule a project discussion?</p>
              <a href="${process.env.VITE_CALENDLY_URL}" style="display:inline-block;background-color:#eab308;color:#0f172a;padding:12px 24px;border-radius:8px;text-decoration:none;font-weight:700;font-size:14px">Schedule a Meeting</a>
            </div>` : ''}
            <p style="font-size:15px;color:#475569;margin-top:32px">Best regards,<br><strong style="color:#0f172a">The HV Construction Team</strong></p>
          </td>
        </tr>
      </table>
    </td></tr>
    ...
    const autoresponderEmail = resend.emails.send({
      ...
      from: "HV Construction <onboarding@resend.dev>", 
      to: [lead.email],
      subject: `Your Project Inquiry is Confirmed — HV Construction`,
...
// Construction project inquiry endpoint (new form)
app.post("/api/inquiry", auditLimiter, async (req, res) => {
  try {
    const body = req.body as Record<string, any>;
    ...
    const projectTypeRaw = body.projectType;
    const projectType = Array.isArray(projectTypeRaw) ? projectTypeRaw.join(", ") : (typeof projectTypeRaw === "string" ? projectTypeRaw.trim() : "");
    const projectLocation = body.projectLocation?.trim() ?? "";
    const description = body.description?.trim() ?? "";

    if (!name || !email || !organization || !projectType || !description) {
      res.status(400).json({ error: "Please complete all required fields." });
      return;
    }
    ...
    const lead: Lead = {
      ...
      estimatedBudget: body.estimatedBudget?.trim() || undefined,
    ...
    // Fire-and-forget: webhook + email
    void (async () => {
      if (WEBHOOK_URL) {
        await fetch(WEBHOOK_URL, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ type: "project_inquiry", lead }),
        }).catch((err) => console.error("[webhook]", err));
      }
      await sendLeadEmail(lead);
    })();

    res.status(201).json({ id: lead.id, message: "Project inquiry received. We'll be in touch shortly." });
  } catch (error) {
...
if (!process.env.VERCEL) {
  app.listen(PORT, () => {
    console.log(`HV Construction API listening on http://localhost:${PORT} [${IS_PROD ? "production" : "development"}]`);
  });
}
