...
export function RootErrorBoundary() {
  ...
  useEffect(() => {
    const savedTheme = window.localStorage.getItem("hv-theme") as "light" | "dark" | null;
  ...
  useEffect(() => {
    ...
    if (isChunkError) {
      ...
      const lastRetry = window.sessionStorage.getItem("hv-chunk-retry");
      const now = Date.now();

      // If last retry was more than 10 seconds ago, retry automatically
      if (!lastRetry || now - parseInt(lastRetry, 10) > 10000) {
        setIsRetrying(true);
        window.sessionStorage.setItem("hv-chunk-retry", String(now));
        ...
        {isRetrying ? (
          <div className="flex flex-col items-center space-y-6">
            ...
            <p className="text-slate-500 dark:text-slate-400 max-w-md">
              We are fetching the latest updates to HV Construction. This will only take a moment.
            </p>
          </div>
        ) : (
          <div className="flex flex-col items-center">
            ...
            <p className="mt-4 text-slate-600 dark:text-slate-300 text-base sm:text-lg max-w-md leading-relaxed">
              We recently updated HV Construction. The browser was trying to load an older version of some files.
            </p>
